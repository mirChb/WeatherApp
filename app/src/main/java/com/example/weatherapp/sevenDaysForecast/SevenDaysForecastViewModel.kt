...
class SevenDaysForecastViewModel() : ViewModel() {
    ...
    fun getDaysForecast(searchItem: String) {
        viewModelScope.launch {
            val details = arrayListOf<SevenDaysCardViewText?>() // Create a new ArrayList to store the items

            for (i in 0..2) {
                try {
                    _properties.value = WeatherApi.reftrofitService.getSevenDaysForecast(searchItem, "7").await()
